...
export function DataProcessFlow() {
  return (
    <section className="py-12 lg:py-24 bg-[#F8F9FA] dark:bg-[#0c0d12] transition-colors duration-200 overflow-x-clip font-sans">
      <div className="container max-w-[1200px] mx-auto px-4 overflow-x-clip">
        ...
        */}
        <div className="hidden lg:block relative w-full max-w-full mx-auto h-[640px] overflow-x-clip overflow-y-visible">

          {/* SVG Connector Layer */}
          <div className="absolute inset-0 pointer-events-none z-0 select-none overflow-x-clip max-w-[1100px] mx-auto">
            <svg className="w-full h-full" viewBox="0 0 1100 640" fill="none" preserveAspectRatio="xMidYMid meet">
          ...
          {/* Top Row Cards */}
          <div className="absolute top-0 left-0 right-0 grid grid-cols-3 gap-6 px-0 pointer-events-none max-w-[1100px] mx-auto">
          ...
          {/* Bottom Row Cards */}
          <div className="absolute bottom-0 left-0 right-0 grid grid-cols-3 gap-6 px-0 pointer-events-none max-w-[1100px] mx-auto">
...
function NodeCard({ node, position }: { node: FlowNode, position: "top" | "bottom" | "mobile" }) {
  ...
  return (
    <div className={`relative flex flex-col h-full w-full items-center ${isMobile ? '' : 'p-2'}`}>
      {/* Card Body */}
      <div className="w-full bg-white dark:bg-[#15161c] rounded-2xl p-5 shadow-[0_2px_20px_-5px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-5px_rgba(0,0,0,0.1)] border border-slate-100 dark:border-gray-800 transition-all duration-300 z-10 flex flex-col group relative overflow-hidden h-full min-h-[180px]">
